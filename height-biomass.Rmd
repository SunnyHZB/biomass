...
*Update*：用median()输出看了一下，Gene171的中位数确实为0没错，但Gene222中位数不为0，只是p值比较接近0.05，第一次置换检验中可能恰好没有被筛选出来。
...
for (i in 1:Z)
{
    ...
    RF=randomForest(Height~.,data=w[-m,],mtry=100,ntree=10000) #用训练集建立模型，mtry是number of predictors sampled for spliting at each node，默认是自变量数/3.
