...
Gene表达量数据由于0比较多，level都少于Height，在将Height划分为有序分类变量时会有一部分信息损失。  
```r
for (i in 1:260)
...
for (i in 1:260)
{
    ...
    Height.ca=GroupNumeric(Height,n=level.num) 
    #使Height的水平个数与该基因相同，GroupNumeric为GoodmanKruskal包里的一个函数，用n=level.num限定对Height分组的组数。
